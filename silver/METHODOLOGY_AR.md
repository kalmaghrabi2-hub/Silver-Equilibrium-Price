...
- شروط المستوى الأساسية: `R² >= 0.60` و`MAPE <= 15%`.
- يضاف Benchmark مستقل: `Naive Persistence`، أي أن توقع الأسبوع التالي يساوي سعر الأسبوع السابق.
- لا ينجح Weekly Gate إلا إذا كان `Skill vs Naive MSE > 0%` وكذلك `Model MAPE < Naive MAPE`.
- يتم حفظ معاملات النموذج ومقاييس الاختبار في `docs/silver/data/weekly_calibration.json`.

هذه الإضافة تمنع اعتبار R² المرتفع وحده دليلًا على وجود قدرة حقيقية للنموذج؛ لأن وجود السعر المتأخر داخل ARX قد يولد R² مرتفعًا حتى لو لم يتفوق النموذج على أبسط توقع استمرارية.

### نتيجة المعايرة الحالية عند الإطلاق — 14 سبتمبر 2026
- إجمالي الأسابيع المكتملة: 1,358.
- التوقعات خارج العينة: 1,202 أسبوعًا.
- `R² = 0.9832`.
- `Model MAPE = 3.753%`.
- `Naive MAPE = 3.369%`.
- `Model RMSE = 1.657 USD/oz`.
- `Naive RMSE = 1.557 USD/oz`.
- `Skill vs Naive MSE = -13.309%`.
- Direction Accuracy = `50.75%`.

لذلك فإن Weekly Gate الحالي = **FAIL** رغم ارتفاع R²، وتبقى النتيجة المركبة **PROVISIONAL**. لم يتم تخفيف الشرط لتمرير النموذج.
...
### Weekly Gate
يصبح `PASS` فقط إذا اجتاز النموذج الاختبار خارج العينة وتفوق على معيار الـNaive وفق MSE وMAPE المحددين مسبقًا.
...
- `PROVISIONAL`: البيانات متاحة لكن إحدى بوابات الحوكمة غير مكتملة أو فشلت.
...
3. تنفيذ walk-forward gate ومقارنة الـNaive benchmark.
...
- لا تعتبر نتيجة `VALID` إذا كان النموذج لا يتفوق على معيار ساذج محدد مسبقًا.
...
- عند ظهور إصدار رسمي أحدث، يجب أن يحل محل snapshot الحالي بعد التحقق من الحقول الرئيسية.
